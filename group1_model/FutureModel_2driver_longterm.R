...
#Plot NEE over time
#plot(phenSite$datetime,phenSite$observation, type="p", pch=1, cex=0.5, main="NEE", xlab="Date", ylab="NEE (gC/m2/day)")
#plot phen vs DAY of Year
#plot(phenDailyAv$julianday,phenDailyAv$PhenAv, type="p", pch=1, cex=0.5, main="NEE", xlab="Day of year", ylab="NEE (gC/m2/day)")
#plot(site_target$datetime, site_target$PhenAv, type="p", pch=1, cex=0.5, main="NEE", xlab="Day of year", ylab="NEE (gC/m2/day)")
...
##sw climatology model for predictions after the forecasts noaa make end
#Add column to sw data with Julian day
swSite<- as.data.frame(site_target_past[,1:4])
swSite$julianday <- yday(swSite$datetime)
##Average sw observations by julian day
swDailyAv <- swSite %>% group_by(julianday) %>% summarise(swAv = mean(downwelling_sw, na.rm = TRUE))

futureswlength<- length(noaa_future_sw$downwelling_sw)
swlastday<- noaa_future_sw[futureswlength, 1]

#extend driver to 365 days
swseqstart<- as.numeric(swlastday+1) #day after our last noaa sw forecast
swseqend<- as.numeric(swlastday+330)
swseq<-seq(from=as.Date(swseqstart, origin = "1970-01-01"), to=as.Date(swseqend, origin = "1970-01-01"), by="day") #days for

swclim_start <- futureswlength+1
noaa_future_sw[swclim_start:365,1] <- swseq #extend sw dates to 365
noaa_future_sw$julianday <- yday(noaa_future_sw$datetime)
noaa_future_sw <- left_join(noaa_future_sw, swDailyAv, by = c("julianday" = "julianday"))
noaa_future_sw[36:365,2] <- noaa_future_sw[36:365,4]
noaa_future_sw<- noaa_future_sw[,1:2]
...
###PHENOLOGY
#Ideally we would use LAI but since we do not have future LAI predictions we are using the phenology climatology predictions
...
future_data[8:372,1]<-noaa_future_sw[1:365,1] #add 35 days of datetime dates into the future to col 1
future_data[8:372,4]<-noaa_future_sw[1:365,2] #add 35 days of sw predictions into the future to col 4
