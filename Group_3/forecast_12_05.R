...
##############MODEL W COVARIATE
##need to download past and future air temp data
...
##Make up air temp data and initial conditions bc we cannot download future data
Temp<-rnorm(966,5,2)
...
#Define  data
data <- list(y=log(y),n=length(y),Temp=Temp,      ## data
             x_ic= dat$chla[694],tau_ic=100, ## initial condition prior
             a_obs=1,r_obs=1,           ## obs error prior
             a_add=1,r_add=1,
             a_driv=1,r_driv=1)
...
plot(time[1:729],ci_chla[2,],type='l',ylim=c(0,123),ylab="Chla-A",xlab="Date",
     xlim=time[time.rng],main= "Model with Temperature")

ecoforecastR::ciEnvelope(time,ci_chla[1,],ci_chla[3,],col=ecoforecastR::col.alpha("lightBlue",0.75))
points(time[1:729],y,pch="+",cex=0.5)

##plot forecast zoomed in
plot(time[1:729],ci_chla[2,],type='l',ylim=c(0,9),ylab="Chla-A",xlab="Date",
     xlim=time[time.rng],main= "Model with Temperature")
...
model_metadata = list(
  forecast = list(
    model_description = list(
      ...
      type = "empirical",  
      repository = "https://github.com/rqthomas/neon4cast-example"   #change?
    ),
